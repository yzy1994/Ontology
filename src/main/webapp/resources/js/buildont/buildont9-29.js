...
//															传递参数应该为单引号：<input type ="button" value ="button" onclick ="test(’note.xml‘);" />
														 ...
														 changeDivLoc(typeDiv,e,typeList.size());
													}									
...
//					}
					...
					//download
					$("#downloadImgId").click(function(){
						
						$.ajax({
							type:"post",
							url:"upload!fileList.action",
							data:"",
							datatype:"json",
													...
													var parem1="download"+"("+i+")";
												        ...
												        for(var j=0;j<5;j++){												        	
												        	   if(j==0){
												        		   ...
												        		   var td=$("<td>"+documentVOList[i].fileName+"</td>");
														           td.appendTo(tr);
//																	var documentLi = $('<li onclick="'+parem1+'";>' + documentVOList[i].fileName + '</li>');	
//																	documentLi.attr('id',i+ "Id");
//																	documentLi.appendTo(td);
													           }else if(j==2){
														           var td=$("<td>"+documentVOList[i].createTime+"</td>");
														           td.appendTo(tr);
													           }else if(j==3){
													               var td=$("<td>"+documentVOList[i].updateTime+"</td>");
														           td.appendTo(tr);
													           }else if(j==4){													        	   
																   var td = $('<td onclick="'+parem1+'";>下载</td>');
														           td.appendTo(tr);
													           }
												        }
												}
											    ...
											    $("tr:gt(0)").find("td:eq(4)").on('mouseover', function () {
											    	this.style.backgroundColor='#ffff66';
											    });
											    $("tr:gt(0)").find("td:eq(4)").on('mouseout', function () {
											    	this.style.backgroundColor='#d4e3e5';
//											    	this.style.backgroundColor='#none';
											    });											    
										    }else{
										    	var message=$("<p>没有语料文件</p>");
										    		message.appendTo($("#corpusListId"));							    	
										    }
							}
...
//				})	
				...
				$('.theme-poptit2 .close8').click(function(){
					...
					function download(i){
						$("#fra").attr("src","download!download?filePath="+documentVOList[i].fileName+"");
...
//							    timeout: 2000,
//							    error: function(xml){
//							          alert('Error loading XML document'+xml.responseText);
//							    },
...
//						  $('#textShow').removeAttr("readonly");
//						  $('#textShow').focus();
//						  var dddd=$("#textShow").val().substr(0,$("#textShow").val().length-4);
...
//					 })
	                ...
	                $("#testId").click(function(){                
						...
						$('#showDiv').slideDown(200);   
						  ...
						  var showSonDiv=document.getElementById ("showSonDiv");
						  var oriHeight=showSonDiv.offsetHeight;
						  
						  var showTitle=document.getElementById ("showTitle");
						  var oriTop=showTitle.offsetTop+showTitle.offsetHeight;
						  var rightTop=oriHeight-oriTop;
						  var rightLeft=showTitle.offsetLeft;
					...
					alert(locList.toString());     
						...
						if(flag==1){
						  ...
						  svgElem.setAttributeNS (null, "x", i*boxWidth+100);             

						  svgElem.style.display = "inline";      
   
	
						 // svgElem.appendChild(rec);
						  ...
						  text.textContent ="中国人";
						  var len=text.textContent.length ;
						  var micX;
						
						  micX=(boxWidth-len*fontsize)/2;
                          
						  text.setAttribute("x",micX);
						  text.setAttribute("y",rightTop+micY);
						  ...
						  var showId = document.getElementById ("ddd");             
						  showId.appendChild (table); 
						}
//						  if(i%20==0)
						  showId.appendChild (document.createElement("br"));
					}
						     //30
...
//	            			   alert(par);
                   ...
                   $("#showId").click(function(){
	                ...
	                $("#saveImgId").click(function(){
	                 ...
	                 $("#modelImgId").click(function(){
	                	 
	                	 var modelImg=$("#modelImgId");
	                	 if(modelState==0){
	                		 modelImg.attr('src', "resources/images/semiAuto.jpg");
	                		 modelState=1;
	                	 }else{
	                		 modelImg.attr('src', "resources/images/manual.jpg");
	                		 modelState=0 ;
	                	 }        
	                 })
...
function saveNoAjax(value,fileName){
	 $.ajax({
			...
			async:false,
			url:"mjolnir_saveTextshow",
			data:{textshowStr:value,docFilename:fileName},
			datatype:"json",
			success:function(data){
				saveResult=true;
			}
...
//												documentLi.appendTo(td);
											   ...
											   var td = $('<td onclick="'+parem1+'";>打开</td>');
									           td.appendTo(tr);
								           }
							        }
							}
